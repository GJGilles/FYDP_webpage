    ...
    <div>
        <ul id="macro-list" class="list-group">
            <li v-for="(macro, index) in macros" :key="macro.id" class="list-group-item row">
                <div class="d-inline-block col-6">{{ macro.name }}</div>
                <button v-on:click="remove(macro.id)" type="button" class="btn btn-danger float-right col-1"><i class="fas fa-trash"></i></button>
                <button v-on:click="edit(index)" type="button" class="btn btn-warning float-right col-1"><i class="fas fa-pencil-alt"></i></button>
                <button v-on:click="run(index)" type="button" class="btn btn-success float-right col-1"><i class="fas fa-play"></i></button>
            </li>
            <li class="list-group-item row">
                <button v-on:click="create()" type="button" class="btn btn-success float-right col-1"><i class="fas fa-plus"></i></button>
            </li>
        </ul>
    </div>
</template>
